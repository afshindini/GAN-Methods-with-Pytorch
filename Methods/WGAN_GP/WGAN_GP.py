...
# Define gradient penalty function to be used with W-loss
def gradient_penalty(cri, real, fake, eps):
    mixed_image = eps * real + (1 - eps) * fake     # Derive a mixed image from fake and real ones
    mixed_critic = cri(mixed_image)     # find the value of critic for mixed image
    gradient = torch.autograd.grad(inputs=mixed_image, outputs=mixed_critic, grad_outputs=torch.ones_like(mixed_critic),
                                   create_graph=True, retain_graph=True)[0]     # calculate related gradient
    gradient = gradient.view(len(gradient), -1)     # Change the view of the gradient
    gradient_norm = gradient.norm(2, dim=1)     # Calculate norm of gradient
    grad_penalty = torch.mean((gradient_norm - 1) ** 2)     # Calculate the gradient penalty
    return grad_penalty
...
def critic_loss(critic_fake_pred, critic_real_pred, gp, c_lambda):
    return -torch.mean(critic_real_pred) + torch.mean(critic_fake_pred) + c_lambda * gp
