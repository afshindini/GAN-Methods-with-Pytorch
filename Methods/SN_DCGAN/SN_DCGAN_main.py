...
from Methods.SN_DCGAN.SN_DCGAN import Generator, Discriminator
from torch.utils.data import DataLoader
from torchvision import transforms
from torchvision.datasets import MNIST
from torch import nn
import torch
from Utility.utility import save_result, plot_loss, weights_init, noise_vector
...
def SN_DCGAN_main():
    ...
    disc = Discriminator(opt.img_channel, int(opt.hidden_dim / 4)).to(opt.device)
    disc_opt = torch.optim.Adam(disc.parameters(), lr=opt.lr, betas=(opt.beta1, opt.beta2))

    # Initial weights for generator and discriminator models
